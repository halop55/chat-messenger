...
import {
  ...
  setMicrophoneMuted,
  setSpeakerEnabled,
} from "./src/services/agoraVoiceService";
import {
  VOICE_CALL_STATUSES,
  acceptVoiceCall,
  declineVoiceCall,
  ...
  startVoiceCall,
  subscribeVoiceCall,
  subscribeVoiceCallInbox,
} from "./src/services/voiceCallServices";
...
const getFriendlyCallError = (error) => {
  ...
  if (error.code === "AGORA_NATIVE_MODULE_UNAVAILABLE") {
    return `Ban can cai lai ban Android native truoc khi test goi thoai. ${getAgoraSetupHelp()}`;
  }

  if (error.code === "AGORA_NATIVE_MODULE_DISABLED") {
    return `${error.message} ${getAgoraSetupHelp()}`;
  }

  if (error.code === "AGORA_EXPO_GO_UNSUPPORTED") {
    return "App van chat duoc trong Expo Go, nhung goi thoai bang Agora chi chay tren ban Android native cai bang npm run android:run.";
  }
...
export function VoiceCallProvider({ children }) {
  ...
  const [voiceCallInbox, setVoiceCallInbox] = useState(null);
  ...
  const voiceCallRef = useRef(voiceCallDoc);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeVoiceCallInbox(currentUserId, (inboxData) => {
      setVoiceCallInbox(inboxData);
    });
  ...
  useEffect(() => {
    const activeChatId = voiceCallInbox?.activeCallId;
    ...
    const unsubscribe = subscribeVoiceCall(activeChatId, (voiceCall) => {
      setVoiceCallDoc(voiceCall);
    });

    return unsubscribe;
  }, [voiceCallInbox?.activeCallId]);
